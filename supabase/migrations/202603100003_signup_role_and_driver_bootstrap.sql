-- Ensure signup requested_role is applied and driver row is bootstrapped.
...
begin
  ...
  set user_id = coalesce(p.user_id, new.id),
      ...
      system_role = case
        when upper(coalesce(p.system_role, 'NONE')) = 'NONE' and requested_role in ('DRIVER', 'FLEET') then requested_role
        else p.system_role
      end
  where p.id = new.id
     or p.user_id = new.id;
