  ...
  try {
    ...
    if (!user) {
      return redirectWithMessage(
        requestUrl,
        `/attorney/login?redirectedFrom=${encodeURIComponent('/api/integrations/google-calendar/connect')}`,
    ...
    return NextResponse.redirect(buildCalendarConnectUrl('GOOGLE', state))
  } catch (error) {
    return redirectWithMessage(
      requestUrl,
      returnTo,
